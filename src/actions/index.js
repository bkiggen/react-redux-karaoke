...
export function fetchSongId(title) {
  return function(dispatch) {
    ...
    return fetch('http://api.musixmatch.com/ws/1.1/track.search?&q_track=' + title + '&page_size=1&s_track_rating=desc&apikey=d38be6bfc7fe97802dbb8ce0ddde404d').then(
      response => response.json(),
      error => console.log('An error occurred.', error)
    ).then(function(json) {
      if (json.message.body.track_list.length > 0) {
        const getInfo = json.message.body.track_list;
        const musicMatchId = getInfo[0].track.track_id;
        const artist = getInfo[0].track.artist_name;
        const title = getInfo[0].track.track_name;
...
export function fetchLyrics(title, artist, musicMatchId, localSongId, dispatch) {
  return fetch('http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=' + musicMatchId + '&apikey=d38be6bfc7fe97802dbb8ce0ddde404d').then(
    response => response.json(),
    error => console.log('An error occurred.', error)
  ).then(function(json){
